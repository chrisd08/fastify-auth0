...
async function plugin(fastify, opts) {
  ...
  if (!(opts.domain && opts.client_id && opts.client_secret)) {
    ...
    throw new Error(
      "domain, client_id, and client_secret are all required options"
    );
  }
  ...
  fastify.route({
    ...
    handler: async function (request, reply) {
      ...
      let userInfo = await utility.getUserInfo(accessToken);
      ...
      if (options.transformer && typeof options.transformer === "function") {
        ...
        userInfo = transformerResults.then
          ? await transformerResults
          : transformerResults;
      ...
      return reply.redirect(
        options.loginSuccessRedirectPath || session.requestPath || "/"
      );
    },
  });
